# 3D map forensics — threejs-liminality

**Status: source-structure study, not a reproduced level.** Maze/room structure is generated in source. Do not use its Backrooms layout; inspect `Maze.js` only for chunking concepts.
...
## Current finding
Maze/room structure is generated in source. Do not use its Backrooms layout; inspect `Maze.js` only for chunking concepts.
