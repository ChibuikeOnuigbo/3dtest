...
The fresh two-seed CI run at [`34049064054`](https://github.com/ChibuikeOnuigbo/3dtest/actions/runs/34049064054) completed smoke/startup steps but the player-capture step exited non-zero. It created **no PNG/player frame** and therefore no visual review or approval. A later diagnostic-only CI adjustment preserved two zero-frame `CAPTURE_ABORTED_PARTIAL` records from this run; they identify the failed `waitForFunction` but are not capture evidence. Normal GitHub log retrieval from this environment returned an Actions-results TLS `EOF`; no bypass or repeated retrieval was used. The next narrowly scoped repair adds the missing live-render/pointer-lock observables to the capture probe. Its outcome remains unverified until a new CI run produces actual frames.

## Visual semantic review — GitHub Actions run 34049702791 default seed (round 03, rejected)

Run [`34049702791`](https://github.com/ChibuikeOnuigbo/3dtest/actions/runs/34049702791) produced eleven actual first-person PNGs from head `42aab0e` under a confirmed pointer lock (`pointer_locked: true`) and live rendered-frame counts. The immutable record is [`default/capture_record.json`](../qa/visual/evidence/34049702791/default/capture_record.json); nine independent reviews cite all eleven exact frames under [`qa/visual/reviews/`](../qa/visual/reviews/). Validator result: `REVIEWED_NOT_APPROVED`, nine roles present, zero metadata findings. This is a semantic reading of the PNGs, not a source/test inference.

`CAPTURED: true` · `INSPECTED: true` · `APPROVED: false`

- **Dispatch (01–03):** the real east/west mouse looks now differ materially, but both reveal bare roof, texture-wrapped brown warehouse masses, pasted teal cards and a blank horizon—not lower rail/harbour/industrial depth. The forward image remains a black/orange stair-bar corridor with a small buried Permit objective.
- **Intake (04–05):** real contact reaches `intake-steps`, a significant capture improvement, but the requested Switch/Transfer has not yet been reached. A dark grated/striped stair field and repeated oxide bands consume the player view.
- **Switch/East (06–07):** actual support is `switch-house` then unsupported air. The close striped post/teal card view gives no East Span take-off, gap or landing; the air frame terminates against a plain brown wall. This is a critical movement/composition failure, not dash evidence.
- **Boiler/Control/Sunline claims (08–11):** the player has reset to `dispatch-bay`, so all four labels are navigation misses. Frame 11 is especially invalid as a final vista: two thick orange foreground bars occlude the lower half and no bridge/horizon is present.
- **All-role action:** retain the honest contact metadata, hide the persistent lower control ribbon, remove eye-level repeated rail/nosing bars, construct open sight-lines with supported service/facade depth, then fix the route/capture phase flow so requested supports are actually reached. Neither current seed is visually approved.

The same CI job captured only one alternate-seed Spawn PNG before its alternate traversal exceeded the job budget. That single rendered image is a real but incomplete seed-comparison artifact, not proof of meaningful variation, a full alternate traversal or an approval. Subsequent CI changes retain trace screenshots while omitting expensive DOM/source snapshots, and limit the alternate run to genuine player-height look-around input so the primary default run remains the authoritative traversal evidence.
